...
$sql =<<<EOF
      select guide_name,phone_number,price from guide where place_id in(select place_id from places where city = '$city') and number_of_teams < 10;
EOF;
...
<form method="post" action="transport.php">
  ...
  <?php
    for($j=($i - 1); $j>=0; $j--) { ?>
      <input type="radio" name="radio" id="radio2" value="<?= $name[$j] ?>"><br><br><?= $phone[$j] ?><br><br><?= $price[$j] ?>
